...
def get_parser():
    ...
    parser.add_argument('--config', type=str,
                    default='config/custom/custom_openseg_pretrained.yaml',
                    help='config file')
    parser.add_argument('opts',
                    default=None,
                    help='see config/custom/custom_openseg_pretrained.yaml for all options',
...
def precompute_text_related_properties(labelset_name):
    ...
    if 'scannet' in labelset_name:
        ...
        palette = get_palette(colormap='matterport_160')
    elif 'custom160' in labelset_name or labelset_name == 'custom_semantic_160':
        labelset = list(CUSTOM_LABELS_160)
        palette = get_palette(colormap='custom_160')
    elif 'custom18' in labelset_name or labelset_name == 'custom_semantic_18':
        labelset = list(CUSTOM_LABELS_18)
        palette = get_palette(colormap='custom_18')
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    datapath_prefix = os.path.join("data", "preprocessed", args.foldername, "custom_3d")
    datapath_prefix_feat= os.path.join(args.data_root_2d_fused_feature, args.foldername)
    val_data = FusedFeatureLoader(datapath_prefix, # datapath_prefix=args.data_root,
    ...
    inference(model, val_loader, labelset_name)

def inference(model, val_data_loader, labelset_name='custom_semantic_18'): # custom_semantic_160
    '''Inference the OpenScene model.'''
    ...
    if args.save_feature_as_numpy: # save point features to folder
        ...
        saved_feature_folder = os.path.join(out_root, args.data_root_2d_fused_feature, 'saved_feature')
        ...
        for rep_i in range(args.test_repeats):
            ...
            for i, (coords, feat, label, feat_3d, mask, inds_reverse) in enumerate(tqdm(val_data_loader)):
                ...
                if feature_type == 'distill': 
                    predictions = model(sinput)  # Pass the input SparseTensor through the model to get 3D feature vectors
                    predictions = predictions[inds_reverse, :]  # Rearrange predictions to match the original point order
                    pred = predictions.half() @ text_features.t()  # Compute similarity between point features and text features (CLIP space)
                    logits_pred = torch.max(pred, 1)[1].cpu()  # Get the class index with the highest similarity for each point
                ...
                if vis_pred:  # using the computed logit index and maps them to the corresponding color 
                ...
                # Visualize GT labels
                if vis_gt:  # using the var label as GT which exists in the pointcloud data
            ...
            if eval_iou:
                print('No eval implemented because there are no GT labels')
