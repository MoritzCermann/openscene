...
from fusion_util import extract_openseg_img_feature, extract_openseg_img_feature_split, PointCloudToImageMapper, save_fused_feature
...
def process_one_scene(data_path, out_dir, args):
    ...
    for img_id, img_dir in enumerate(tqdm(img_dirs)):
        ...
        if keep_features_in_memory:
            ...
            feat_2d = extract_openseg_img_feature_split(img_dir, openseg_model, text_emb).to(device)
            print(f"feat_2d Shape: {feat_2d.shape} put together from 6 patches")  # Should be (C, 1280, 1920)
...
def main(args):
    ...
    ##### Dataset specific parameters ##### landscape -> (1920, 1280)
    img_dim = (1920, 1280)  # 640 360
    depth_scale = 1000
    ...
    data_root = join(data_dir, 'custom_3d')
    data_root_2d = join(data_dir,'custom_2d')
